...
inactive, which includes every serial run. \texttt{local\_max\_sweeps} bounds a same-rank
...
\item \textbf{Results are not bit-identical between the two.} Photons arrive in a different
order, so each block consumes its random stream differently. Escaping fractions agree
within Poisson noise, which is the most that can be asked.
